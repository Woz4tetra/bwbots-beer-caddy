...
class SimpleGoToPose:
    def __init__(self) -> None:
        rospy.init_node("simple_go_to_pose")
    ...
    def compute_goal(self, msg: PoseStamped) -> Optional[Pose2d]:
        ...
        if msg.header.frame_id == self.robot_parent_frame_id:
            ...
            return cast(Pose2d, pose2d.transform_by(self.robot_state))
        else:
            rospy.logwarn(
                "Moving goal frame id doesn't match odometry. Can't set goal. " f"Received {msg.header.frame_id}."
            )
            return None
    ...
    def action_callback(self, goal: GoToPoseGoal):
        ...
        while current_time - start_time < timeout:
            ...
            if is_done:
                rospy.loginfo(f"Controller finished. Pose error: {self.goal_pose - self.robot_state}")
                success = True
                break
