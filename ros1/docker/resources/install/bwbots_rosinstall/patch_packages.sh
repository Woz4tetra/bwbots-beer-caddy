...
git checkout -f
git apply /root/install/evergreen/fix-image-pipeline.patch --reject --whitespace=fix 

cd ${DEP_ROS_WS_SRC}/geometry2/
git checkout -f
git apply /root/install/evergreen/fix-geometry2.patch --reject --whitespace=fix 

cd ${DEP_ROS_WS_SRC}/imu_tools/
git checkout -f
git apply /root/install/evergreen/fix-imu-tools.patch --reject --whitespace=fix 
