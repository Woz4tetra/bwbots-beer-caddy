...
BwDescription::BwDescription(ros::NodeHandle* nodehandle):nh(*nodehandle)
{
    ...
    for (int index = 0; index < _joint_names.size(); index++)
    {
        ...
        raw_joint_subs->push_back(
            nh.subscribe<std_msgs::Float64>(
                ...
                boost::bind(&BwDescription::joint_callback, this, _1, index)
            )
        );
    }
    ...
    ROS_INFO("bw_description is ready!");
}

void BwDescription::joint_callback(const std_msgs::Float64ConstPtr& msg, int joint_index)
{
