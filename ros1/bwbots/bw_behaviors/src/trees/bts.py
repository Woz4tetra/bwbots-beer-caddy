...
class BehaviorTrees:
    def __init__(self) -> None:
        ...
        self.has_drink_mass_threshold: float = rospy.get_param("~has_drink_mass_threshold", 0.025)
        self.tag_prep_offset: float = rospy.get_param("~tag_prep_offset", -0.7)
        self.valid_tag_window: float = rospy.get_param("~valid_tag_window", 1.0)
        self.tag_settle_time: float = rospy.get_param("~tag_settle_time", 1.5)
    ...
    def follow_tag(self, tag_name_supplier, x_offset, y_offset, theta_offset):
        return FollowTagBehavior(x_offset, y_offset, theta_offset, tag_name_supplier, self.tag_manager)
    ...
    def search_for_tag(self, tag_name_supplier):
        return py_trees.composites.Parallel(
            ...
            children=[
                ...
                StopDrivingDecorator(py_trees.decorators.FailureIsRunning(self.find_tag(tag_name_supplier)))
            ]
        )
    ...
    def is_tag_near(self, tag_name_supplier):
        return IsTagNearBehavior(self.robot_frame, tag_name_supplier, self.tag_manager, 1.0)
    
    def save_tag_as_waypoint(self, x_offset, y_offset, theta_offset, tag_name_supplier, waypoint_name_supplier):
        return SaveTagAsWaypointBehavior(x_offset, y_offset, theta_offset, self.global_frame, waypoint_name_supplier, tag_name_supplier, self.tag_manager)
    ...
    def dispenser_type_supplier(self):
        def supplier():
            ...
            if mission is None:
                ...
                return self.tag_manager.get_tag(self.drink_mission_manager.get_active().drink_dispenser_tag).tag_type
        return supplier()
    ...
    def go_to_dispenser_type_A0_stage2(self):
        return py_trees.composites.Sequence("Park under A0 stage2", [
            GoToTagBehavior(
                -0.2,
                -0.25,
                -math.pi / 2.0,
                ...
                allow_reverse=False,
                failure_on_pose_failure=False,
                valid_tag_window=rospy.Duration(60.0)
            ),
            GoToTagBehavior(
                -0.15,
                -0.05,
                ...
                allow_reverse=False,
                failure_on_pose_failure=False,
                valid_tag_window=rospy.Duration(80.0)
            ),
        ])
    ...
    def go_to_dispenser_type_A0_stage3(self):
        return GoToTagBehavior(
            ...
            allow_reverse=False,
            failure_on_pose_failure=False,
            valid_tag_window=rospy.Duration(100.0)
        )
        ...
        return py_trees.composites.Sequence("Dock", [
            ...
            RepeatNTimesDecorator(py_trees.composites.Selector("Search tag stage 1", [
                py_trees.composites.Sequence("Go to tag stage 1", [
                    self.go_to_dock_stage1(), PauseBehavior(self.tag_settle_time), self.find_tag(self.dock_tag_supplier)
                ]),
                self.search_for_tag(self.dock_tag_supplier)
            ]), attempts=2),
    ...
    def undock(self):
        return py_trees.composites.Sequence("Undock", [
            ...
            #     self.is_tag_near(self.dock_tag_supplier),
            # ]),
            self.save_tag_and_prep_waypoints(self.tag_prep_offset, 0.0, self.dock_tag_supplier, self.dock_prep_supplier),
        ])
    
    def find_and_save_tag_and_prep_waypoints(self, prep_x_offset: float, prep_y_offset: float, tag_waypoint_supplier, prep_waypoint_supplier):
        return py_trees.composites.Sequence("Find and save tag and prep waypoints", [
            self.find_tag(tag_waypoint_supplier),
            self.save_tag_and_prep_waypoints(prep_x_offset, prep_y_offset, tag_waypoint_supplier, prep_waypoint_supplier)
        ])
    
    def save_tag_and_prep_waypoints(self, x_offset: float, y_offset: float, tag_waypoint_supplier, prep_waypoint_supplier):
        return py_trees.composites.Sequence("Save tag and prep waypoints", [
            self.save_tag_as_waypoint(0.0, 0.0, 0.0, tag_waypoint_supplier, tag_waypoint_supplier),
            self.save_tag_as_waypoint(x_offset, y_offset, 0.0, tag_waypoint_supplier, prep_waypoint_supplier),
        ])
    ...
    def dispenser_stage1_sequence(self):
        ...
        return py_trees.composites.Sequence("Go to dispenser stage 1", [
            ...
            self.save_tag_and_prep_waypoints(self.tag_prep_offset, 0.0, self.dispenser_tag_supplier, self.dispenser_prep_supplier),
        ])
    ...
    def dispenser_stage3_sequence(self):
        return SelectBySupplierDecorator({
            "A0": self.go_to_dispenser_type_A0_stage3()
        }, self.dispenser_type_supplier)
