...
class Joystick:
    ...
    def stop(self):
        ...
        self.logger.info("Set joystick thread stop flag")
    ...
    def open_joystick(self):
        try:
            ...
            self.logger.info("%d axes found: %s" % (self.num_axes, ", ".join(self.axis_map)))
            self.logger.info("%d buttons found: %s" % (self.num_buttons, ", ".join(self.button_map)))
    ...
    def update(self):
        if not self.task_running():
            self.logger.error("Error detected in read task. Raising exception")
    ...
    def update_task(self, should_stop):
        try:
            while True:
                if should_stop():
                    self.logger.info("Exiting joystick thread\n\n")
                    return

                self.check_joystick_events()

        except BaseException as e:
            self.logger.error("An exception occurred in the joystick thread", exc_info=True)
